...
MCMC.RMWEXP.NonAdapt<-function(N,thin,Q,beta0,gam0,Time,Cens,X,hyp1.gam=1,hyp2.gam=1,ar=0.44,omega2.beta,omega2.gam=1,EXP=FALSE)
{
  ...
  for(iter in 2:(N+1))
  {
    ...
    if(EXP==FALSE)
    {
      MH.gam=GRWMH.RMW.gam(N=1,omega2=omega2.gam,gam0=gam.aux,Time=Time,Cens=Cens,X=X,beta=beta.aux,theta=NA,lambda=lambda.aux,typ.theta=NA,hyp.theta=NA,hyp1.gam=hyp1.gam,hyp2.gam=hyp2.gam,mixing='None',lower.bound=0.06)
      gam.aux<-MH.gam$gam
  ...
  chain=cbind(beta,gam,lambda)
  return(chain)
}

# REDUCED MCMC RUN WITH FIXED GAMMA (MARGINAL LIKELIHOOD CALCULATION)
MCMCR.RMWEXP.gam<-function(N,thin,Q,beta0,gam0,lambda0,Time,Cens,X,ar=0.44,omega2.beta)
{
  k<-length(beta0); n<-length(Time); N.aux<-N/thin
  beta<-matrix(rep(0,times=(N.aux+1)*k),ncol=k); beta[1,]<-beta0
...
# REDUCED RUN MCMC WITH FIXED beta[1],...,beta[J] (MARGINAL LIKELIHOOD CALCULATION)
MCMCR.RMWEXP.betaJ.gam<-function(N,thin,Q,beta0,gam0,lambda0,Time,Cens,X,J,ar=0.44,omega2.beta)
{
  k<-length(beta0); n<-length(Time); N.aux<-N/thin
  beta<-matrix(rep(0,times=(N.aux+1)*k),ncol=k); beta[1,]<-beta0
  gam<-rep(gam0,times=N.aux+1)
...
GRWMH.RMWGAM.theta<-function(N=1,omega2,theta0,gam,lambda,type.prior,hyper)
{
  n<-length(lambda); theta<-rep(0,times=N+1); theta[1]<-theta0; ind<-rep(0,N+1)
  for(l in 1:N)
  {
    y<-rnorm(n=1,mean=theta[l],sd=sqrt(omega2))
    if(y<=2/gam) {theta[l+1]<-theta[l]; ind[l+1]<-2}
    else
    {
      u.aux<-runif(1,min=0,max=1)
      aux1=n*(y*log(y)-lgamma(y)-theta[l]*log(theta[l])+lgamma(theta[l]))+(y-theta[l])*sum(log(lambda))-(y-theta[l])*sum(lambda)
      aux2=log(prior.theta(theta=y,gam=gam,a=hyper,type=type.prior,mixing="Gamma")/prior.theta(theta=theta[l],gam=gam,a=hyper,type=type.prior,mixing="Gamma"))
      log.aux<-aux1+aux2
...
# NON-ADAPTIVE MCMC (MARGINAL LIKELIHOOD CALCULATION)
MCMC.RMWGAM.NonAdapt<-function(N,thin,Q,beta0,gam0,theta0,Time,Cens,X,typ.theta,hyp.theta,hyp1.gam=1,hyp2.gam=1,ar=0.44,omega2.beta,omega2.gam=1,omega2.theta,EXP=FALSE)
{
  k<-length(beta0); n<-length(Time); N.aux<-N/thin
  beta<-matrix(rep(0,times=(N.aux+1)*k),ncol=k); beta[1,]<-beta0
  ...
  for(iter in 2:(N+1))
  {
    ...
    if(EXP==FALSE)
    {
      MH.gam=GRWMH.RMW.gam(N=1,omega2=omega2.gam,gam0=gam.aux,Time=Time,Cens=Cens,X=X,beta=beta.aux,theta=theta.aux,lambda=lambda.aux,typ.theta=typ.theta,hyp.theta=hyp.theta,hyp1.gam=hyp1.gam,hyp2.gam=hyp2.gam,mixing="Gamma",lower.bound=0.06)
      gam.aux<-MH.gam$gam
      if(MH.gam$ind==1) {accept.gam=accept.gam+1}
    }

    MH.theta=GRWMH.RMWGAM.theta(N=1,omega2=omega2.theta,theta0=theta.aux,gam=gam.aux,lambda=lambda.aux,type.prior=typ.theta,hyper=hyp.theta)
...
MCMCR.RMWGAM.theta<-function(N,thin,Q,beta0,gam0,theta0,lambda0,Time,Cens,X,typ.theta,hyp.theta,hyp1.gam=1,hyp2.gam=1,ar=0.44,omega2.beta,omega2.gam=1,EXP=FALSE)
{
  k<-length(beta0); n<-length(Time); N.aux<-N/thin
  beta<-matrix(rep(0,times=(N.aux+1)*k),ncol=k); beta[1,]<-beta0
  gam<-rep(0,times=N.aux+1); gam[1]<-gam0
...
MCMCR.RMWGAM.gam.theta<-function(N,thin,Q,beta0,gam0,theta0,lambda0,Time,Cens,X,typ.theta,hyp.theta,hyp1.gam,hyp2.gam,ar=0.44,omega2.beta)
{
  k<-length(beta0); n<-length(Time); N.aux<-N/thin
  beta<-matrix(rep(0,times=(N.aux+1)*k),ncol=k); beta[1,]<-beta0
  gam<-rep(gam0,times=N.aux+1)
...
MCMCR.RMWGAM.betaJ.gam.theta<-function(N,thin,Q,beta0,gam0,theta0,lambda0,Time,Cens,X,J,typ.theta,hyp.theta,hyp1.gam,hyp2.gam,ar=0.44,omega2.beta)
{
  k<-length(beta0); n<-length(Time); N.aux<-N/thin
  beta<-matrix(rep(0,times=(N.aux+1)*k),ncol=k); beta[1,]<-beta0
  gam<-rep(gam0,times=N.aux+1)
...
# GAUSSIAN RANDOM WALK METROPOLIS STEP FOR THETA
GRWMH.RMWIGAM.theta<-function(N=1,omega2,theta0,gam,lambda,type.prior,hyper)
{
  n<-length(lambda); theta<-rep(0,times=N+1); theta[1]<-theta0; ind<-rep(0,N+1)
  ...
  {
    y<-rnorm(n=1,mean=theta[l],sd=sqrt(omega2))
    if(y<=1) {theta[l+1]<-theta[l]; ind[l+1]<-2}
    else
    {
      u.aux<-runif(1,min=0,max=1)
      aux1=n*(lgamma(theta[l])-lgamma(y))+(theta[l]-y)*sum(log(lambda))
      aux2=log.prior.theta(theta=y,gam=gam,a=hyper,type=type.prior,mixing="InvGamma")-log.prior.theta(theta=theta[l],gam=gam,a=hyper,type=type.prior,mixing="InvGamma")
      log.aux<-aux1+aux2
      if(is.na(log.aux)==TRUE){print("fail theta"); print(c(theta[l],y,aux1,aux2))}
      if(log(u.aux)<log.aux) {theta[l+1]<-y; ind[l+1]<-1}
      else {theta[l+1]<-theta[l]; ind[l+1]<-0}
    }
  }
...
# NON-ADAPTIVE MCMC (MARGINAL LIKELIHOOD CALCULATION)
MCMC.RMWIGAM.NonAdapt<-function(N,thin,Q,beta0,gam0,theta0,Time,Cens,X,typ.theta,hyp.theta,hyp1.gam=1,hyp2.gam=1,ar=0.44,omega2.beta,omega2.gam=1,omega2.theta,EXP=FALSE)
{
  k<-length(beta0); n<-length(Time); N.aux<-N/thin
  beta<-matrix(rep(0,times=(N.aux+1)*k),ncol=k); beta[1,]<-beta0
  theta<-rep(0,times=N.aux+1); theta[1]<-theta0
  ...
  for(iter in 2:(N+1))
  {
    ...
    if(EXP==FALSE)
    {
      MH.gam=GRWMH.RMW.gam(N=1,omega2=omega2.gam,gam0=gam.aux,Time=Time,Cens=Cens,X=X,beta=beta.aux,theta=theta.aux,lambda=lambda.aux,typ.theta=typ.theta,hyp.theta=hyp.theta,hyp1.gam=hyp1.gam,hyp2.gam=hyp2.gam,mixing="InvGamma",lower.bound=0.06)
      gam.aux<-MH.gam$gam
      if(MH.gam$ind==1) {accept.gam=accept.gam+1}
    }

    MH.theta=GRWMH.RMWIGAM.theta(N=1,omega2=omega2.theta,theta0=theta.aux,gam=gam.aux,lambda=lambda.aux,type.prior=typ.theta,hyper=hyp.theta)
    theta.aux<-MH.theta$theta
    if(MH.theta$ind==1) {accept.theta=accept.theta+1}
...
# REDUCED MCMC RUN WITH FIXED THETA (MARGINAL LIKELIHOOD CALCULATION)
MCMCR.theta.RMWIGAM<-function(N,thin,Q,beta0,gam0,theta0,lambda0,Time,Cens,X,typ.theta,hyp.theta,hyp1.gam=1,hyp2.gam=1,ar=0.44,omega2.beta,omega2.gam=1,EXP=FALSE)
{
  k<-length(beta0); n<-length(Time); N.aux<-N/thin
  beta<-matrix(rep(0,times=(N.aux+1)*k),ncol=k); beta[1,]<-beta0
  gam<-rep(0,times=N.aux+1); gam[1]<-gam0
...
# REDUCED MCMC RUN WITH FIXED THETA AND GAMMA (MARGINAL LIKELIHOOD CALCULATION)
MCMCR.gam.theta.RMWIGAM<-function(N,thin,Q,beta0,gam0,theta0,lambda0,Time,Cens,X,typ.theta,hyp.theta,hyp1.gam,hyp2.gam,ar=0.44,omega2.beta)
{
  k<-length(beta0); n<-length(Time); N.aux<-N/thin
  beta<-matrix(rep(0,times=(N.aux+1)*k),ncol=k); beta[1,]<-beta0
  gam<-rep(gam0,times=N.aux+1)
...
# REDUCED RUN MCMC WITH FIXED THETA, GAMMA AND beta[1],...,beta[J] (MARGINAL LIKELIHOOD CALCULATION)
MCMCR.betaJ.gam.theta.RMWIGAM<-function(N,thin,Q,beta0,gam0,theta0,lambda0,Time,Cens,X,J,typ.theta,hyp.theta,hyp1.gam,hyp2.gam,ar=0.44,omega2.beta)
{
  k<-length(beta0); n<-length(Time); N.aux<-N/thin
  beta<-matrix(rep(0,times=(N.aux+1)*k),ncol=k); beta[1,]<-beta0
  gam<-rep(gam0,times=N.aux+1)
