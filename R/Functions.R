...
#' @title CDF prediction based on a pre-computed MCMC chain.
#'
#' @description This functions returns a Monte Carlo estimate of the predictive CDF \eqn{P(T < t | D)}, based on a (pre-computed)
#' posterior sample of all model parameters of the associated RMW regression model (obtained based on a dataset \eqn{D}).
#'
#' @param Chain List containing the pre-computed MCMC sample (as generated by the function \code{RMWreg_MCMC}).
#' @param Time Time threshold \eqn{t} at which the predictive CDF is estimated.
#' @param x Vector of covariate information for the new subject (must be in the same order as in the
#' design matrix used to fit the model, including a '1' on the first position to represent the intercept).
#' @param Mixing Mixing distribution assigned to the (frailty) random effects. Possible values are
#' \code{"None", "Exponential", "Gamma", "InvGamma", "InvGauss", "LogNormal"}
#' @param BaseModel If \code{BaseModel = "Weibull"}, a RMW regression is used. If \code{BaseModel = "Exponential"}, a RME regression is used.
#'
#' @return A Monte Carlo estimate of the predictive CDF.
#'
#' @examples
#'
#' library(KMsurv)
...
#'
#' @author Catalina A. Vallejos \email{cvallejos@@turing.ac.uk}
RMWreg_PredictCDF <- function(Chain, Time, x, Mixing = "None", BaseModel = "Weibull")
{
  message(sprintf("Only predictions for BaseModel = 'Weibull' and Mixing = 'None' are available in this implementation"))

  beta <- Chain$beta
  if(BaseModel == "Weibull") {gam = Chain$gam}

  AuxPred = rep(0, times = length(gam))
  for(i in 1:length(gam))
  {
    alpha.aux = exp(- gam[i] * t(x) %*% beta[i,] )
    AuxPred[i] = 1 - exp(- alpha.aux * Time^(gam[i]))   # Weibull CDF as parametrized in the RMWreg library.
  }
